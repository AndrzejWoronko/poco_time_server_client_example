...
protected:
    ...
    int main(const std::vector<std::string>& args)
    {
        ...
        else
        {
            ...
            std::string format(config().getString("TimeServer.format", DateTimeFormat::SORTABLE_FORMAT));
            std::string addr(config().getString("TimeServer.address", "127.0.0.1"));
            IPAddress ipa(addr);
            try
            {
                ipa = ipa.parse(addr);
            ...
            std::cout << "Host from config: " << addr << std::endl;
            SocketAddress sa(ipa, port);
            // set-up a server socket
            ServerSocket svs(sa);
            // set-up a TCPServer instance
            TCPServer srv(new TimeServerConnectionFactory(format), svs);

            std::cout << "Starting on adress: " << sa.toString() << std::endl;
            std::cout << "Time format: " << format << std::endl;
